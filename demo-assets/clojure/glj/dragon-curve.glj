; https://rosettacode.org/wiki/Dragon_curve#Clojure
...
(defn bit-count [n]
  (loop [i 0
         count 0]
    (if (< i 64)
      (recur (inc i) (+ count (bit-and 1 (bit-shift-right n i))))
      count)))

(defn i->dir
  [n]
  (mod (bit-count (bit-xor n (bit-shift-right n 1))) 4))

(defn safe-bit-or [v bit] (bit-or (or v 0) bit))

(defn -main [& args]
  (let [steps (if (seq args) (parse-long (first args)) 511)
